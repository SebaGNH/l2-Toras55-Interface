...
function OnLoad()
{
	...
	Load();
	m_StatusIconSongDance.SetAnchor("PetStatusWnd.StatusIconBuff", "TopLeft", "TopLeft", 0, 0);
}
...
function UpdateBuff()
{
	...
	{
		// 버프 , 디버프 보여 준다.
		m_StatusIconDeBuff.SetAnchor("PetStatusWnd.StatusIconBuff", "TopLeft", "TopLeft", 0, buffCount*24);
...
function OnRButtonDown( WindowHandle a_WindowHandle, int X, int Y )
{
	...
	{
			if (!class'UIAPI_WINDOW'.static.IsShowWindow("PetWnd"))
			{
...
{	
	...
	{
		if ((skillInfo.IsDebuff == false && skillInfo.OperateType == 1) || (skillInfo.SkillID == 1323))		{	RequestDispel(info.ServerID, info.ID, info.Level);	}					//버프 취소 요청
