...
function MoveItemBottomToTop( int index, bool bAllItem )
{
	...
	{
		if( !bAllItem && IsStackableItem( bottomInfo.ConsumeType ) && (bottomInfo.ItemNum > IntToInt64(1)) )		// 개수 물어보기
		{
...
function HandleOpenWindow( string param )
{
	...
	//branch
	{
		...
		m_EnchantWnd = GetHandle( "ItemEnchantWnd" );		//인챈트 윈도우 핸들을 얻어온다.
