...
var ButtonHandle			m_lvlButton[NPARTYSTATUS_MAXCOUNT];
...
var WindowHandle 		cb_Crown;
//var TextureHandle		cb_CrownTex;
...
var int			isMOver[NPARTYSTATUS_MAXCOUNT];
...
function OnLoad ()
{
  ...
  cb_Crown.SetTooltipCustomType(MakeTooltipSimpleText("Alt+Drag to change party leader"));	
...
function InitHandle ()
{
  ...
  cb_Crown = GetHandle("PartyWnd.QuickOptionsWnd.KingOfTheHill");
//  cb_CrownTex = TextureHandle(GetHandle("PartyWnd.QuickOptionsWnd.KingOfTheHill.KingOfTheHillTex"));
  for (idx=0; idx<NPARTYSTATUS_MAXCOUNT; idx++)
  {
...
//	PlayerLevel[idx] = TextBoxHandle(GetHandle("PartyWnd.PartyStatusWnd" $ string(idx) $ ".LevelBox.txtLvl"));
	m_lvlButton[idx] = ButtonHandle(GetHandle("PartyWnd.PartyStatusWnd" $ string(idx) $ ".btnLvl"));
//	m_lvlButton[idx].SetTexture("","","");
//	m_lvlButton[idx].HideWindow();
  }
}
...
function InitHandleCOD ()
{
  ...
  cb_Crown = GetWindowHandle("PartyWnd.QuickOptionsWnd.KingOfTheHill");
//  cb_CrownTex = GetTextureHandle("PartyWnd.QuickOptionsWnd.KingOfTheHill.KingOfTheHillTex");
...
//	PlayerLevel[idx] = GetTextBoxHandle("PartyWnd.PartyStatusWnd" $ string(idx) $ ".LevelBox.txtLvl");
	m_lvlButton[idx] = GetButtonHandle("PartyWnd.PartyStatusWnd" $ string(idx) $ ".btnLvl");
//	m_lvlButton[idx].SetTexture("","","");
//	m_lvlButton[idx].HideWindow();
  }
}
...
//  local string parentname;
  ...
  if ( Event_ID == 1140 )
  {
	...
	script.HandleTargetUpdate();
  } else if ( Event_ID == 1150 )
  {
    HandlePartyUpdateParty(param);
//	if (isPartyMember(script.UserName.GetName())) script.btnInvite.HideWindow();
//	else script.btnInvite.ShowWindow();
  } else if ( Event_ID == 1160 )
  {
    HandlePartyDeleteParty(param);
	if (AmILeader()) cb_Crown.ShowWindow();
	UpdateCBAssist(m_CurCount);
	script.HandleTargetUpdate();
...
// if (m_lvlButton[idx].isShowwindow()) 
//	 m_lvlButton[idx].HideWindow();
  UpdateCPBar(idx,0,0);
  UpdateHPBar(idx,0,0);
  UpdateMPBar(idx,0,0);
...
function CopyStatus(int DesIndex, int SrcIndex)
{
	...
	//			m_lvlButton[DesIndex].GetTextureName();
	m_lvlButton[DesIndex].SetTexture("l2ui_ct1.emptyBtn","l2ui_ct1.emptyBtn","l2ui_ct1.emptyBtn");
	Level = m_lvlButton[SrcIndex].GetButtonName();
	m_lvlButton[DesIndex].SetNameText(Level);
...
function bool isPartyMember(string Name)
{
	local int idx;
	local bool check;
	check = false;
	for (idx=0; idx<NPARTYSTATUS_MAXCOUNT; idx++)
	{
		if (m_PlayerName[idx].GetName() == Name)
		{
			if (m_PartyStatus[idx].isShowWindow()) check = true;
		}
	}
	return check;
}
...
function ChangeMemberIcons()
{
	...
	for (idx = 0; idx < m_CurCount; idx++)
	{
		...
		if (class'UIAPI_CHECKBOX'.static.IsChecked("PartyWndOption.showNewIcons"))
		{
			...
			m_DetailedClassIcon[idx].SetTexture(GetDetailedClassIconName(memInfo.nSubClass, 1));
...
//	ParseInt(param, "Nobless", Nobless);
	...
	if (ParseInt(param, "MasterID", MasterID))
	{
		...
		{	
			...
			cb_Crown.SetTooltipCustomType(MakeTooltipSimpleText(GetRoutingString(RoutingType)));
	...
	//?? ???
	if (class'UIAPI_CHECKBOX'.static.IsChecked("PartyWndOption.showNewIcons") && GetDetailedClassIconName(ClassID, 1) != "None")
		{
			m_ClassIcon[idx].SetTexture("");
			m_ClassIcon[idx].HideWindow();
			m_DetailedClassIcon[idx].ShowWindow();
			m_DetailedClassIcon[idx].SetTexture(GetDetailedClassIconName(ClassID, 1));
...
//	m_lvlButton[idx].ShowWindow();
	m_lvlButton[idx].SetTexture("l2ui_ct1.emptyBtn","l2ui_ct1.emptyBtn","l2ui_ct1.emptyBtn");
	if (isMOver[idx] == 0) m_lvlButton[idx].SetNameText(string(Level));
...
function OnClickButtonWithHandle(ButtonHandle Button)
{
	local int idx;
	local string parentName;
	parentName = Button.GetParentWindowName();
//	AddSystemMessageString("kek");
//	AddSystemMessageString(Button.GetParentWindowName());
	if (( InStr( parentName , "PartyStatusWnd" ) > -1 ) && (Button.GetWindowName()=="btnLvl"))
	{//?? ??? ?? ??
			idx = int(Right(parentName , 1));
//			AddSystemMessageString(string(idx));
			ExecuteCommand("/dismiss " $ m_PlayerName[idx].GetName() );
//			AddSystemMessageString(m_PlayerName[idx].GetName());
//				//	parentname.GetParentWindowName();
//			ResizeWnd();
	}
//	for (idx=0; idx<NPARTYSTATUS_MAXCOUNT; idx++)
//	{
//		if (Button == m_lvlButton[idx]) 
//		{
//			AddSystemMessageString(string(idx));
//			ExecuteCommand("/dismiss " $ m_PlayerName[idx].GetName() );
//			AddSystemMessageString(m_PlayerName[idx].GetName());
...
//  local string parentname;
  ...
  switch (strID)
  {
    ...
    break;
//	case "btnLvl":
//	 AddSystemMessageString(strID);
//	 if ( InStr(strID,"PartyStatusWnd") > -1 )
//	 {
//		idx = int(Right(strID,17));
...
function OnLButtonDown (WindowHandle a_WindowHandle, int X, int Y)
{
  ...
  if ( (X > rectWnd.nX + 13) && (X < rectWnd.nX + rectWnd.nWidth - 30) && (Y - rectWnd.nY > 0) )
  {
...
//	keka.B = 0;
	...
	CurrentWndName = a_WindowHandle.GetWindowName();
	ParentWndName = a_WindowHandle.GetParentWindowName();
//	AddSystemMessageString(CurrentWndName);
//	AddSystemMessageString(ParentWndName);
	if (cb_Crown.isShowWindow())
	{
		if(Right(CurrentWndName,3) == "Lvl")
			{
				idx = int(Right(ParentWndName,1));
				buttonLvl = m_lvlButton[idx].GetButtonName();
				isMOver[idx] = 1;
				m_lvlButton[idx].SetTooltipCustomType(MakeTooltipSimpleText("Dismiss" @ m_PlayerName[idx].GetName() ));
	//			m_PlayerName[idx].SetNameWithColor(m_PlayerName[idx].GetName(),NCT_Normal,TA_Center, keka); 
				m_lvlButton[idx].SetNameText("-");			
			}
	}
	else 
	{
		if(Right(CurrentWndName,3) == "Lvl")
			{
				...
				buttonLvl = m_lvlButton[idx].GetButtonName();
				isMOver[idx] = 1;
				m_lvlButton[idx].SetTooltipCustomType(MakeTooltipSimpleText(""));
	//			m_PlayerName[idx].SetNameWithColor(m_PlayerName[idx].GetName(),NCT_Normal,TA_Center, keka); 
	//			m_lvlButton[idx].SetNameText("-");			
			}
	}
...
//	AddSystemMessageString(ParentWndName);
		...
		if(Right(CurrentWndName,3) == "Lvl")
			{
				...
				m_lvlButton[idx].SetTooltipCustomType(MakeTooltipSimpleText("Dismiss" @ m_PlayerName[idx].GetName() ));
				isMOver[idx] = 0;
				m_lvlButton[idx].SetNameText(buttonLvl);
				//m_PlayerName[idx].SetNameWithColor(m_PlayerName[idx].GetName(),NCT_Normal,TA_Center, white); 		
			}
	}
	else
	{
	...
	//}
//	if ( Event_ID == EV_MouseOut )
//	{
//		ParseString(param, "Name", CurrentWndName);
//		ParseString(param, "TopWndName", ParentWndName);
//		AddSystemMessageString(CurrentWndName);
//		AddSystemMessageString(ParentWndName);
//		if(Right(CurrentWndName,3) == "Lvl")
//		{
//			idx = int(Right(ParentWndName,1));
//			m_lvlButton[idx].SetNameText(level);			
//		}
//	}
...
//	 AddSystemMessageString(sTargetName);
//	 AddSystemMessageString(sDropName);
//	 AddSystemMessageString(sTargetParent);
	// PartyStatusWnd?? ??? ? ??? ? ? ??. 
	//if(( InStr( "PartyStatusWnd", sTargetName ) == -1 ) || ( InStr( "PartyStatusWnd" ,sDropName) == -1 ))
	
	if (sDropName =="KingOfTheHill")
	{
		if( InStr( sTargetName , "PartyStatusWnd" ) > -1 ) 	//?? ??? ?? ??
		{
			targetIdx = int(Right(sTargetName , 1));
			ExecuteCommand("/changepartyleader" @  m_PlayerName[targetIdx].GetName());
		}
		else									//?? ??? ??? ??? ??? PartyStatusWnd ??
		if( InStr( sTargetParent , "PartyStatusWnd" ) > -1 )
		{
			targetIdx = int(Right(sTargetParent , 1));
			ExecuteCommand("/changepartyleader" @  m_PlayerName[targetIdx].GetName());
		}
	//	 AddSystemMessageString("/changepartyleader" @  m_PlayerName[targetIdx].GetName());	
//		ExecuteCommand("/changepartyleader" @  m_PlayerName[targetIdx].GetName());
	}
