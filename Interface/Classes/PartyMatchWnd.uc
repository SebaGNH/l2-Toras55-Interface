...
//
	//		AddSystemMessageString( "[" $ i $ "]" $ "[" $ j $ "]==" $ MemberName );
	//		AddSystemMessageString( "[" $ i $ "]" $ "[" $ j $ "]==" $ MemberClassID );
	//		AddSystemMessageString( "[" $ i $ "]" $ "[" $ j $ "]==" $ Nobless );
	//		AddSystemMessageString( "[" $ i $ "]" $ "[" $ j $ "]==" $ IDD );
...
function OnClickListCtrlRecord( String a_ListCtrlName )
{
	local int SelectedRecordIndex;
	local LVDataRecord Record;	
	...
	if( a_ListCtrlName == "PartyMatchListCtrl" )
	{
		if ( IsKeyDown(IK_Shift) )
		{
			...
			{
			SetChatMessage( "\"" $ Record.LVDataList[2].szData $ " " );
			}
		}
	}
	return;
