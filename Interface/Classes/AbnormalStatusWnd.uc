...
function HandleAddNormalStatus(string param)
{
	...
	for (i=0; i<Max; i++)
	{
		...
		if (IsValidItemID(info.ID))
		{
			...
			else if (IsTriggerSkill(info.ID, info.Level) || (info.ID.ClassID == 1323 && noblesseType == 2))
			{
			...
			else if (IsDebuff( info.ID, info.Level) || (info.ID.ClassID == 1323 && noblesseType == 1))
			{
				if (!class'UIAPI_CHECKBOX'.static.IsChecked( "FlexOptionWnd.showDebuffTimer" ))
				{
...
//		Me.SetWindowSize(rectWnd.nWidth + NSTATUSICON_FRAMESIZE, rectWnd.nHeight);
		...
		//세로 프레임 사이즈 변경
		if (class'UIAPI_WINDOW'.static.IsShowWindow("DebuffWnd"))
		{
			Me.SetWindowSize(rectWnd.nWidth + NSTATUSICON_FRAMESIZE, rectWnd.nHeight + 26);
			Me.SetFrameSize(NSTATUSICON_FRAMESIZE, rectWnd.nHeight + 26);
		}
		else
		{
			Me.SetWindowSize(rectWnd.nWidth + NSTATUSICON_FRAMESIZE, rectWnd.nHeight);
