...
function OnLoad()
{
	if(CREATE_ON_DEMAND==0)
		OnRegisterEvent();
	Me = GetWindowHandle("PartyMatchRoomWnd");
	...
	class'UIAPI_BUTTON'.static.HideWindow( "PartyMatchRoomWnd.ThiefButton" );
}
...
function UpdateMyMembershipType()
{
	...
	case 2:		
		...
		class'UIAPI_BUTTON'.static.HideWindow( "PartyMatchRoomWnd.ReCreateButton" );
	...
	case 1:
		...
		if (!Recreate) class'UIAPI_BUTTON'.static.ShowWindow( "PartyMatchRoomWnd.ReCreateButton" );		
		class'UIAPI_BUTTON'.static.EnableWindow( "PartyMatchRoomWnd.ExitButton" );
		break;
	}
}
...
function AddMember( int a_MemberID, string a_MemberName, int a_ClassID, int a_Level, int a_ZoneID, int a_MembershipType, string a_RestrictZoneID )
{
	...
	Record.LVDataList[1].szTexture = GetDetailedClassIconName( a_ClassID, 0 );
	Record.LVDataList[1].nTextureWidth = 14;
	Record.LVDataList[1].nTextureHeight = 14;
...
function OnClickButton( string a_strButtonName )
{
	...
	case "ExitButton":
		...
		break;
	case "ReCreateButton":
		OnReCreateButtonClick();
		break;
	case "ShoutButton":
		ProcessChatMessage( "!" $ class'UIAPI_TEXTBOX'.static.GetText( "PartyMatchRoomWnd.RoomTitle"), 0);
		ProcessChatMessage( "+" $ class'UIAPI_TEXTBOX'.static.GetText( "PartyMatchRoomWnd.RoomTitle"), 0);
		break;

//	case "ThiefButton":
		//class'UIAPI_WINDOW'.static.ShowWindow( "ThiefPartyWnd" );
		//AddSystemMessageString("Under Construction");
//		break;				
	}
}

function OnReCreateButtonClick()
{
	local PartyMatchMakeRoomWnd Script;
	local String MaxPartyMemberCount;
	local String MinLevel;
	local String MaxLevel;
	local String RoomTitle;
	local String tmp;
	
	MaxPartyMemberCount = class'UIAPI_TEXTBOX'.static.GetText( "PartyMatchRoomWnd.PartyMemberCount" );
	MaxPartyMemberCount = Right(MaxPartyMemberCount,2);
	if( InStr(MaxPartyMemberCount,"/") != -1) MaxPartyMemberCount = Right(MaxPartyMemberCount,1);
	tmp = class'UIAPI_TEXTBOX'.static.GetText( "PartyMatchRoomWnd.LevelLimit" );
//	AddSystemMessageString(tmp);
	MinLevel = Left(tmp,2);
	if( InStr(MinLevel,"-") != -1) MinLevel = Left(tmp,1);
	MaxLevel = Right(tmp,2);
	if( InStr(MaxLevel,"-") != -1) MinLevel = Right(tmp,1);

//	MinLevel = class'UIAPI_TEXTBOX'.static.GetText( "PartyMatchMRoomWnd.LevelLimit" );
//	MaxLevel = class'UIAPI_TEXTBOX'.static.GetText( "PartyMatchMRoomWnd.LevelLimit" );
	RoomTitle = class'UIAPI_TEXTBOX'.static.GetText( "PartyMatchRoomWnd.RoomTitle");
	
//	AddSystemMessageString(string(RoomNumber));
	
	class'PartyMatchAPI'.static.RequestDismissPartyRoom( RoomNumber );
	Script = PartyMatchMakeRoomWnd( GetScript( "PartyMatchMakeRoomWnd" ) );
	Script.SetRoomNumber( 0 );

	class'PartyMatchAPI'.static.RequestManagePartyRoom( Script.RoomNumber, int(MaxPartyMemberCount),int(MinLevel), int(MaxLevel), RoomTitle );
	//AddSystemMessageString(InvitedName);
	//class'PartyMatchAPI'.static.RequestAskJoinPartyRoom( InvitedName );
	class'UIAPI_WINDOW'.static.SetFocus( "PartyMatchRoomWnd" );
	class'UIAPI_BUTTON'.static.HideWindow( "PartyMatchRoomWnd.ReCreateButton" );
	Me.SetTimer(6671,60000);
	Recreate = true;
}
...
function OnTimer(int TimerID)
{
	...
	if (TimerID == 6671)
	{
		if (MyMembershipType==1) class'UIAPI_BUTTON'.static.ShowWindow( "PartyMatchRoomWnd.ReCreateButton" );
		Me.KillTimer(6671);
	}
