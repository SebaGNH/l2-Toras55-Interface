...
var bool RecStart, isWin, isBurning, isOnsl, gotDMG1, gotDMG2; // optCheck, 
//var int c_counter;
...
function OnLoad() {
	...
	LFTex = GetTextureHandle("OlympiadDmgWnd.LifeForce");
...
function UpdateVp (int Vitality )
{
	...
	else if(Vitality >= 17000) 	//활력 4단계. 경험치 보너스 300%. 사이값 3000
	{
		...
		pVP.SetValue(3000,Vitality-17000);
		
	}
	else if(Vitality >=13000)	// 활력 3단계. 경험치 보너스 250%. 사이값 4000
	{
		LFTex.SetTexture("pug.lf03");
        ...
        pVP.SetValue(4000,Vitality-13000);
	}
	else if(Vitality >= 2000)		//활력 2단계. 경험치 보너스 200%. 사이값 11000 
	{
		LFTex.SetTexture("pug.lf02");
        ...
        pVP.SetValue(11000,Vitality-2000);
	}
	else if(Vitality >= 240)		//활력 1단계. 경험치 보너스 150%. 사이값 1760
	{
		LFTex.SetTexture("pug.lf01");
        ...
        pVP.SetValue(1760,Vitality-240);
	}
...
function OptionBtnShow()
{
	...
	local RecommendBonusWnd script2;
	local PCCafeEventWnd script3;
	local AdenaInfoWnd script4;
	script = OlyDmgOptionWnd( GetScript( "OlyDmgOptionWnd" ) );
	script2 = RecommendBonusWnd( GetScript( "RecommendBonusWnd" ) );
	script3 = PCCafeEventWnd( GetScript( "PCCafeEventWnd" ) );
	script4 = AdenaInfoWnd( GetScript( "AdenaInfoWnd" ) );
	
	if (!script.Me.IsShowWindow())
	{
		script.Me.ShowWindow();
		
		if (script2.Me.IsShowWindow())
			 script2.Me.HideWindow();
		if (script3.Me.IsShowWindow())
			 script3.Me.HideWindow();
		if (script4.Me.IsShowWindow())
			 script4.Me.HideWindow();
		//For sudden(1)
	}
	else
	{
		...
		if (!script2.Me.IsShowWindow())
			 script2.Me.ShowWindow();
		if (!script3.Me.IsShowWindow())
			 script3.Me.ShowWindow();
		if (!script4.Me.IsShowWindow())
			 script4.Me.ShowWindow();
		//For sudden(2)
	}
}
   ...
   if (Event_ID == EV_OlympiadTargetShow) 
	{
		...
		if (class'UIAPI_CHECKBOX'.static.IsChecked("FlexOptionWnd.enableAutoSS"))
		{
			...
			ss_script.EnterTheVoid();
		}
		if (GetPetInfo(pussy_MyPet) && (ss_script.ShowPet)) ss_script.EnterTheAbyss();		
        TimeRemaining = 360;
		OlympiadStuff();
        Me.ShowWindow();
        Me.SetTimer(TIMER_ID, TIMER_DELAY);
	...
	//	Me.SetTimer(1234, 6000);
		...
		SetFieldInfo();}
		else if (Event_ID == EV_BR_PREMIUM_STATE) {
		HandlePremiumState(param);}
}
...
function HandlePremiumState( String a_Param )
{
	...
	if (m_CurPremiumState == 1) {
		...
		pName10.SetTextColor(keka);
		pName12.SetTextColor(keka);
		pLVL.SetTextColor(keka);
		//PlayAnimationPrem();
	}
	else {
		//LevelBoxTex.ShowWindow();
		pLVL.SetTextColor(White);
		pName10.SetTextColor(White);
		pName12.SetTextColor(White);
	}
}
...
//	}
	...
	if (TimerID == 10)
	{
		...
		buffCount = 0;
		if (pussy_MyPet.Name == "Feline Queen") 	BuffMe();	
	}
...
//	local int i;
	...
	if ((DefenderInfo.Name == "Olympiad Host") || (DefenderInfo.Name == "Imperial Phoenix") || (DefenderInfo.Name == "Dark Panther") || (DefenderInfo.Name == "Mew the Cat") || (DefenderInfo.Name == "Nightshade") || (DefenderInfo.Name == "Magnus the Unicorn") || (DefenderInfo.Name == "Mechanic Golem") || (DefenderInfo.Name == "Divine Beast") || (DefenderInfo.Name == "Unicorn Seraphim") || (DefenderInfo.Name == "Feline Queen") || (DefenderInfo.Name == "Reanimated Man") || (DefenderInfo.Name == "Kai the Cat") || (DefenderInfo.Name == "Boxer the Unicorn"))
		return;
...
function HandleReceiveMagicSkillUse (string a_Param)
{
  ...
  if ( ((SkillID == 662) && (PlayerInfo.nID != AttackerID)) &&  !IsNotDisplaySkill(SkillID) ) //DEnchanter
  {
	...
	{
		for (i = 0; i < summnames.Length; i++)
			if (summnames[i] == AttackerInfo.Name)
			{
	...
	{
		for (i = 0; i < summnames.Length; i++)
			if (summnames[i] == DefenderInfo.Name)
			{
...
function HandleReceiveAttack (string a_Param)
{
  ...
  if ( PlayerInfo.nID != AttackerID && DefenderInfo.nID != mypet_ID && ( (DefenderInfo.Name == "Dark Panther") || (DefenderInfo.Name == "Imperial Phoenix") || (DefenderInfo.Name == "Divine Beast") ) )
  {
    Charging = true;
