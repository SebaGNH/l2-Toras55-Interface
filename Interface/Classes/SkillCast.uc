...
//			return;
	...
	if (  !IsNotDisplaySkill(SkillID) && (AttackerID == PlayerInfo.nID) && (TargetInfo.nID == PlayerInfo.nID ))
	{
		SetCastInfo(AttackerInfo,UsedSkillInfo,SkillHitTime_ms);
	}		
}
...
function SetCastInfo (UserInfo DefenderInfo, SkillInfo CastInfo, int SkillHitTime_ms)
{
	...
	Me.SetWindowSize(nWndWidth-1, 36);
	...
	if (CastInfo.EnchantName == "none") skillName.SetText(CastInfo.SkillName);
	else skillName.SetText(CastInfo.SkillName $" " $ "("$ CastInfo.EnchantName $")" );
