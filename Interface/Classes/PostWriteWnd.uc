...
{
	if(ControlName == "AccompanyItem" || ControlName == "InventoryItem")
	{
	...
	if( IsKeyDown( IK_Alt ))
	{
		if(ControlName == "InventoryItem")
		{
			if ( ZoneType == 12 )
			UseInvenToAccompany( index, true );
		}
		else if(ControlName == "AccompanyItem")
		{
			UseAccompanyToInven( index, true );
		}		
	}
}
*/
...
//	itemAmountList.Remove(0, itemAmountList.Length);
	if (PostTitle.GetString() != "mail") named = true;
	titlepost = "";
	for (i = 0; i < AccompanyItem.GetItemNum(); i++)
	{		
//		AddSystemMessageString("named:"  $ named $ ".");
		AccompanyItem.GetItem(i, iteminfo);
		if ((iteminfo.Name != "") && !named)
		{
			
			if  (titlepost == "") 
			{
				titlepost = iteminfo.Name;
				if (iteminfo.ItemNum > IntToInt64(1)) titlepost = (titlepost $ "[" $ MakeCostString(Int64ToString(iteminfo.ItemNum)) $ "]");	
			}
			else
			{			
				titlepost = titlepost $ ", "  $ iteminfo.Name;
				if (iteminfo.ItemNum > IntToInt64(1)) titlepost = titlepost $ "[" $ MakeCostString(Int64ToString(iteminfo.ItemNum)) $ "]";	
				if 	(Len(titlepost) >= 50) titlepost = Left(titlepost , 47) $ "...";
			}			
//			AddSystemMessageString("titlepost:"  $ titlepost $ ".");	
			PostTitle.SetString(titlepost);
		}
