...
var ButtonHandle btnInstance;
var ButtonHandle btnOly;
...
var ItemControlWnd script_items;
...
function OnLoad()
{
	...
	if(CREATE_ON_DEMAND==0)		// e©ø¨ùe¡¾¡Æi? i?¢¬e¡°¢´ e¡Æ?e¢¬¡Æ
	{
		Me = GetHandle( "ShortcutWnd" );
		m_hFlightShipWnd=GetHandle("FlightShipCtrlWnd");
		m_hFlightCtrlWnd=GetHandle("FlightTransformCtrlWnd");
		btnInstance = ButtonHandle(GetHandle("ShortcutWnd.btnInstance"));
		btnOly = ButtonHandle(GetHandle("ShortcutWnd.btnOly"));
		btnOpenPanel  = ButtonHandle(GetHandle("ShortcutWnd.btnOpenPanel"));
		btnOpenPanel1  = ButtonHandle(GetHandle("ShortcutWnd.btnOpenPanel1"));
		btnClosePanel  = ButtonHandle(GetHandle("ShortcutWnd.btnClosePanel"));
		btnClosePanel1  = ButtonHandle(GetHandle("ShortcutWnd.btnClosePanel1"));
	}
	else	// i??e¢®©«i?¢¥ i?¢¬e¡°¢´ e¡Æ?e¢¬¡Æ!!
	{
		...
		btnInstance = GetButtonHandle("ShortcutWnd.btnInstance");
		btnOly = GetButtonHandle("ShortcutWnd.btnOly");
		btnOpenPanel = GetButtonHandle("ShortcutWnd.btnOpenPanel");
		btnOpenPanel1 = GetButtonHandle("ShortcutWnd.btnOpenPanel1");
		btnClosePanel = GetButtonHandle("ShortcutWnd.btnClosePanel");
		btnClosePanel1 = GetButtonHandle("ShortcutWnd.btnClosePanel1");
	}
	
	script_autoss = AutoSS(GetScript("AutoSS"));
	script_autosoul = AutoSouls(GetScript("AutoSouls"));
	script_items = ItemControlWnd(GetScript("ItemControlWnd"));
	...
	btnInstance.SetTooltipCustomType(MakeTooltipSimpleText("Instance Zone" ));
	btnOly.SetTooltipCustomType(MakeTooltipSimpleText("Olympiad Arena" ));
...
{	
	...
	case EV_ShortcutCommandSlot:
		...
		break;
	case EV_ShortcutPageUpdate:					//i©÷¡ìe©÷?i¡×¢¬i?i¡í¡¤i¡Æ¨öi? i??i¢¥i¡×¢æe¡Æ¢æ i??e¢®©« i?¢´i??e?i??i? e?¨« e??e¨ùi?¢´e?¡± i¢¥e©÷¢´i?¢¬
...
function AddChatInfoZone()
{
	local Color ColorMsg;
	local ChatWnd script2;	// ??? ???? ???
	
	//script1 = PartyWnd( GetScript("PartyWnd") );
	script2 = ChatWnd( GetScript("ChatWnd") );	
	ColorMsg.R = 120;
	ColorMsg.G = 120;
	ColorMsg.B = 120;
	
	script2.SystemMsg.AddString( "> ---------- Instance Zone ---------- <", ColorMsg );
	ExecuteCommand("/instancezone");
//	test = GetTotalSceneTime();
//	AddSystemMessageString(string(int(test*1000)));
//	script2.SystemMsg.AddString( "> -------------------------------------- <", ColorMsg );
...
function SetCurPage( int a_nCurPage )
{
	...
	//e?¡§i¢Ò?i?¢´e?¡± i©÷¡ìe©÷?i¡×¢¬i?i¡í¡¤i¡Æ¨öe¡×¨« i¡Æ¢¬i¢®¡Æi??e?¢æe¢®©«..
	class'ShortcutAPI'.static.SetShortcutPage( a_nCurPage );
	
	//->EV_ShortcutPageUpdate e¡Æ¢æ e??e¨ùi?¡§e?¢´.
}
...
function OnClickExpandShortcutButton()
{
	...
	if (!m_IsVertical)
	{
		if (m_IsExpand5)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndHorizontal",-2, -30, false);
		...
		else if (m_IsExpand4)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndHorizontal_5", -2, -30, false);
		...
		else if (m_IsExpand3)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndHorizontal_4", -2, -30, false);
		...
		else if (m_IsExpand2)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndHorizontal_3",-2, -30, false);
		...
		else if (m_IsExpand1)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndHorizontal_2", -2, -30, false);
		...
		else
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndHorizontal_1", -2, -30, false);
	...
	else
	{
		if (m_IsExpand5)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndVertical", -50, 462, true);
		...
		else if (m_IsExpand4)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndVertical_5", -50, 462, true);
		...
		else if (m_IsExpand3)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndVertical_4", -50, 462, true);
		...
		else if (m_IsExpand2)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndVertical_3", -50, 462, true);
		...
		else if (m_IsExpand1)
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndVertical_2", -50, 462, true);
		...
		else
		{
			...
			AnchorSouls("ShortcutWnd.ShortcutWndVertical_1", -50, 462, true);
...
function ExecuteShortcutCommandBySlot(string param)
{
	...
	if(Me.isShowwindow())		// i¡Æ¨öi¢¥ e©ø¢¥i?¡þi¡×? e?¨«e¡×¨« i??i?¢¶i??e?e¢® i©÷?e|¡þ.
	{
...
function AnchorShots(string window, int x, int y)
{
	...
	isVert = GetOptionBool( "Game", "IsShortcutWndVertical" );
	if (!isVert) AutoSS.SetAnchor(window, "TopLeft", "TopLeft", x-1, y+4);
	else AutoSS.SetAnchor(window, "TopLeft", "TopLeft", x+4, y-1);

}

function AnchorSouls(string window, int x, int y, bool V)
{
		script_autosoul.Me.SetAnchor(window, "TopRight", "TopRight", x, y);	
}
...
function AnchorItems(string window, int x, int y)
{
	local bool isVert;	
	isVert = GetOptionBool( "Game", "IsShortcutWndVertical" );
	if (!isVert) script_items.Me.SetAnchor(window, "TopLeft", "TopLeft", x+36, y+10);
	else script_items.Me.SetAnchor(window, "TopLeft", "TopLeft", x+10, y+40);
