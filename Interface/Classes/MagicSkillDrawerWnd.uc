...
//var int RootAfterFail;
var int IndexAfterFail;
// 현재 상태의 스킬 정보
var int curSkillID; 
var int curLevel;
...
var bool isAuto;
var int value;
var int CodexValue, MasteryValue, OblivionValue, DisciplineValue;
...
function OnClickButton( string Name )
{
	...
	if ( InStr( Name ,"ResearchRoot_1_") > -1 )
	{
		...
		IndexAfterFail = int(Right(Name, 1));
		rootID = int(Right(Name, 1));
		if (!isAuto) saveRoute = IndexAfterFail;
...
function OnTimer( int TimerID )
{
	...
	{
		case 7776:
	//	AddSystemMessageString("7776 work");
		Me.KillTimer(7776);
		if (curLevel != value)
			OnClickStart();
		else
...
function OnClickStart()
{
	...
	if (IndexAfterFail != -1)
	{
	//	AddSystemMessageString("1RequestExEnchantSkillInfoDetailbefore");
		RequestExEnchantSkillInfoDetail(EnchantState, curWantedSkillID, curWantedLevel);
	//	AddSystemMessageString("1RequestExEnchantSkillInfoDetailafter");
	//	AddSystemMessageString("EnchantState" @ EnchantState @ "curWantedSkillID" @ curWantedSkillID @ "curWantedLevel" @ curWantedLevel);
	...
	value = (IndexAfterFail+1) * 100 + value;
...
//	AddSystemMessageString("EnchantState" @ EnchantState @ "curWantedSkillID" @ curWantedSkillID @ "curWantedLevel" @ curWantedLevel @ "curLevel" @ curLevel @ "value" @ value);
	if (value < curLevel)
	{
	//	AddSystemMessageString("ya tut");
		OnClickStop();
		DialogShow( DIALOG_Modalless, DIALOG_OK, "Value less than current level!" );
		return;
	}
	else if (curWantedLevel != 0 && curLevel != value)
	{
		Me.KillTimer(7776);
		btnStart.HideWindow();
		btnStop.ShowWindow();
	//	AddSystemMessageString("tochim DO po usloviuy curWantedLevel != 0 && curLevel != value" @ EnchantState @ "curWantedSkillID" @ curWantedSkillID @ "curWantedLevel" @ curWantedLevel @ "curLevel" @ curLevel @ "value" @ value);
		RequestExEnchantSkill(EnchantState, curWantedSkillID, curWantedLevel);
	//	AddSystemMessageString("tochim PoSLE po usloviuy curWantedLevel != 0 && curLevel != value" @ EnchantState @ "curWantedSkillID" @ curWantedSkillID @ "curWantedLevel" @ curWantedLevel @ "curLevel" @ curLevel @ "value" @ value);
...
function int CalculateResourses()
{
	...
	local int index_codex, index_mastery, index_oblivion, index_discipline;
	local ItemID item_id_codex, item_id_mastery, item_id_oblivion, item_id_discipline;
	local ItemInfo codexInfo, masteryInfo, oblivionInfo, disciplineInfo;
	...
	item_id_oblivion.ClassID = 9625;
	item_id_discipline.ClassID = 9626;
	
	index_codex = i_handle.Finditem(item_id_codex);
	index_mastery = i_handle.Finditem(item_id_mastery);
	index_oblivion =  i_handle.Finditem(item_id_oblivion);
	index_discipline = i_handle.Finditem(item_id_discipline);
	
	i_handle.GetItem(index_codex, codexInfo);
	i_handle.GetItem(index_mastery, masteryInfo);
	i_handle.GetItem(index_oblivion, oblivionInfo);
	i_handle.GetItem(index_discipline, disciplineInfo);
	
	CodexValue =  Int64ToInt(codexInfo.ItemNum);
	MasteryValue =  Int64ToInt(masteryInfo.ItemNum);
	OblivionValue =  Int64ToInt(oblivionInfo.ItemNum);
	DisciplineValue =  Int64ToInt(disciplineInfo.ItemNum);
//	PrintItemInfo(codexInfo);
...
//	AddSystemMessageString("Simple ENch");
	RequestExEnchantSkill(EnchantState, curWantedSkillID, curWantedLevel); // zatochka
...
function OnEvent(int Event_ID, String param)
{
	...
	switch (Event_ID)
	{
		...
		//	AddSystemMessageString("EV_SkillEnchantInfoWndShow");
			OnEVSkillEnchantInfoWndShow(param);
			break;
		case EV_SkillEnchantInfoWndAddSkill:
			CalculateResourses();
		//	AddSystemMessageString("EV_SkillEnchantInfoWndAddSkill");
			OnEVSkillEnchantInfoWndAddSkill(param);
			if (isAuto)
			{
		//		AddSystemMessageString(string(isAuto));
				OnClickButton("ResearchRoot_1_"$IndexAfterFail);
			}
			if ((!isAuto) && (saveRoute != -1))
			{
		//		AddSystemMessageString(string(isAuto));
				OnClickButton("ResearchRoot_1_" $ saveRoute);
			}
			break;
	//		DisableChecks();
	...
	//		break;
		...
		//	AddSystemMessageString("EV_SkillEnchantInfoWndAddExtendInfo");
			OnEVSkillEnchantInfoWndAddExtendInfo(param);
			break;
		case EV_SkillEnchantResult:
		//	AddSystemMessageString("EV_SkillEnchantResult");
			OnEVSkillEnchantResult(param);
			break;
		case EV_DialogOK:
		//	AddSystemMessageString("EV_DialogOK");
			HandleDialogOK();
			break;
		case EV_DialogCancel:
		//	AddSystemMessageString("EV_DialogCancel");
			HandleDialogCancel();
			break;
	}
}
...
function OnEVSkillEnchantResult(string param)
{
	...
	ParseInt(param, "success", iSuccess);
			...
			//	AddSystemMessageString("Success");
	...
	if ( EnchantState == ENCHANT_NORMAL )
		{
			...
			EnchantMaterialInfo[0].SetText(GetSystemString(1514)@string(codexNum) @ "(" $ string(CodexValue) $ ")");
		...
		} else if ( EnchantState == ENCHANT_SAFETY )
		{
			...
			//비전서 아이템 표시
			EnchantMaterialName[0].SetText(info_a.Name);
			EnchantMaterialInfo[0].SetText(GetSystemString(1514)@string(codexNum) @ "(" $ string(MasteryValue) $ ")");
			EnchantMaterialIcon[0].Clear();
			EnchantMaterialIcon[0].Additem(info_a);
		...
		} else if ( EnchantState == ENCHANT_UNTRAIN)
		{
			...
			//비전서 아이템 표시
			EnchantMaterialName[0].SetText(info_a.Name);
			EnchantMaterialInfo[0].SetText(GetSystemString(1514)@string(codexNum) @ "(" $ string(OblivionValue) $ ")");
			EnchantMaterialIcon[0].Clear();
			EnchantMaterialIcon[0].Additem(info_a);
		...
		}else if ( EnchantState == ENCHANT_ROOT_CHANGE)
		{
			...
			//비전서 아이템 표시
			EnchantMaterialName[0].SetText(info_a.Name);
			EnchantMaterialInfo[0].SetText(GetSystemString(1514)@string(codexNum) @ "(" $ string(DisciplineValue) $ ")");
			EnchantMaterialIcon[0].Clear();
			EnchantMaterialIcon[0].Additem(info_a);
