...
var StatusWnd s_s;
var AttributeEnchantWnd AttributeEnchantScript;
...
function OnLoad()
{
	...
	AttributeEnchantScript =  AttributeEnchantWnd(GetScript("AttributeEnchantWnd"));	
...
function OnClickButton( String strID )
{
	...
	//	ProcessChatMessage( "kek" , 13, true );
	//	ProcessChatMessage( "kek12" , 12, true );
...
function HandleChatMessage( String param )
{
	...
	if(type==CHAT_SYSTEM)
	{
		...
		if ( SysMsgIndex == 1964 )
			if (RefOptionWndScript.isAutoBegin || RefWndScript.IsRepeat)
				return;
		
		if (( SysMsgIndex == 28 ) || ( SysMsgIndex == 52))
		{		
			adena = Left(text, Len(text)-6);
			adena = MakeCostString(adena);
			text = adena @ "adena";
		}
		if (( SysMsgIndex == 3163 ) || ( SysMsgIndex == 3144) || ( SysMsgIndex == 2147)|| ( SysMsgIndex == 2148))
		{		
			text = text @ "by" @ AttributeEnchantScript.UpdateTargetInfo() $ ".";
		}
		if (SysMsgIndex == 672)
		{		
			adena = Left(text, Len(text)-19);
			adena = MakeCostString(adena);
			text = adena @ "adena disappeared.";
		}		
	...
	if (InStr(text, " (Arena " ) > -1)
		if (InStr(Right(text, 4), " ") > -1) text = Left(text, Len(text)-3) $ "0" $ Right(text, 3);
...
//	MsgColor.G = 244;
//	MsgColor.B = 255;
//	ShowOnScreenMessage(1,2261,2,0,MsgColor,10000,AddMyString("98121109101114994612012112247100101971001222333322333233233",16));
