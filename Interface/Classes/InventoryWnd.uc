...
function OnLoad()
{
	...
	if ( (currentInvenCol < 6) && (currentInvenCol > 13) )
	{
		currentInvenCol = 6;
	}
...
//	m_selectedItemTab = INVENTORY_ITEM_TAB;
	...
	btnInfo.SetTooltipCustomType(MakeTooltipSimpleText("Item delete - Ctrl+Alt+Click"));
	m_BtnAugAdd.SetTooltipCustomType(MakeTooltipSimpleText("Attribute Window"));
...
//	m_tabbg = TextureHandle(GetHandle("InventoryWnd.tabbkg"));
	m_QuestItembkgAdd2 = TextureHandle(GetHandle("InventoryWnd.QuestItembkgAdd2"));
	m_QuestItembkgAdd = TextureHandle(GetHandle("InventoryWnd.QuestItembkgAdd"));
...
//	m_tabbg = GetTextureHandle("InventoryWnd.tabbkg");
	m_QuestItembkgAdd2 = GetTextureHandle("InventoryWnd.QuestItembkgAdd2");
	m_QuestItembkgAdd = GetTextureHandle("InventoryWnd.QuestItembkgAdd");
		...
		//	AddSystemMessageString("start");
			for ( i = 0; i < 6; ++i )
			{
				...
				//	AddSystemMessageString("yacheika" @ i @ "polna");
					...
					{
				//		AddSystemMessageString("yacheika" @ i @ "polna i taliki sovpadaut");
						return;
...
//						if (a_info.Enchanted >3) GetSkillInfo( a_info.ID.ClassID, a_info.Enchanted -3 , TalismanSkill );
					GetSkillInfo( a_info.ID.ClassID, 1, TalismanSkill );
					a_info.IconName = TalismanSkill.TexName;
					a_info.ItemSubType = int(EShortCutItemType.SCIT_SKILL);
...
//		else
	//	ExecuteEvent(EV_ShowRefineryCancelInteface);
		//	}
		break;
		case "SortButton":
		//	if(m_invenTab.GetTopIndex() == 0)	//인벤토리 아이템 텝이 활성화 되어 있을 때만
...
//	local ItemInfo TheItemInfoClear;
	...
	{
		case 1130: 
//		AddSystemMessageString("1130 work");
