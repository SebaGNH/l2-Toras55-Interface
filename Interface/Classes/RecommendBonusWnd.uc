...
var TextBoxHandle NavitTimer;                          // 보너스 시간 표시
var string NavitTimerText;
...
function OnLoad()
{
	...
	NavitTimer = GetTextboxHandle("RecommendBonusWnd.NavitTimer");
...
function OnEvent( int a_EventID, String a_Param )
{
	switch( a_EventID )
	{
		...
		case EV_GamingStateEnter			: class'UIAPI_WINDOW'.static.ShowWindow("RecommendBonusWnd");break;
	}
}
...
function HandleNavitPointInfo( String a_Param )
{
	...
	if( !m_bNavit )
	{	NavitTimerText = String( percent )$"%";
		NavitTimer.SetText(NavitTimerText);
...
function TimerNavitToolTip()
{
	...
	NavitTimerText = String( m_NavitEffectRemainSec );
	NavitTimer.SetText(NavitTimerText);
}
...
function NavitGaugeChange( int level, bool forceChange )
{
	...
	if (level>12) texName = "BranchSys2.ui.Br_NavitPoint_12";
	if( ( forceChange == true ) || ( m_Navit_Level != level ) )
	{
			
		Navit_Tex_Gauge.SetTexture( texName );
		Navit_Tex_Gauge.SetAlpha(255);
